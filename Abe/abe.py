...
ABE_URL = 'https://github.com/OroOrg/abe-oro-explorer'

COPYRIGHT_YEARS = '2011'
COPYRIGHT = "Abe developers"
COPYRIGHT_URL = 'https://github.com/bitcoin-abe'

DONATIONS_BTC = '3Cd2xHKiXzAcbM86mC8PKdnyxZxXnNSc9n'
...
<head>
    ...
    <title>Oro Block Explorer</title>
</head>
<body>
...
class Abe:
	...
	body += [
            '<table>\n',
	    '<tr><th></th><th>BLOCK CHAIN</th><th>EXCHANGES</th><th>E-COMMERCE</th><th>TOTAL</th>',
            '</tr>\n']

	chain = abe.store.get_chain_by_name("Oro")
	decimals = DEFAULT_DECIMALS if chain.decimals is None else chain.decimals
	coin = 10 ** decimals

        data = {}
	with open('data_cache.py') as json_file:  
    	    data = json.load(json_file)

        tx_day = data['block']['day']
	tx_week = data['block']['week']
        tx_month = data['block']['month']
	tx_year = data['block']['year']
        tx_total = data['block']['total']
	Tx_day = format_float(tx_day)
        Tx_week = format_float(tx_week)
	Tx_month = format_float(tx_month)
        Tx_year = format_float(tx_year)
	Tx_total = format_float(tx_total)

        usd_day = format_float(data['exchanges']['day'])
	usd_week = format_float(data['exchanges']['week'])
        usd_month = format_float(data['exchanges']['month'])
	usd_year = format_float(data['exchanges']['year'])
        usd_total = format_float(data['exchanges']['total'])

	total_day = tx_day + data['exchanges']['day'] + data['e-commerce']['day']
        total_week = tx_week + data['exchanges']['week'] + data['e-commerce']['week']
	total_month = tx_month + data['exchanges']['month'] + data['e-commerce']['month']
        total_year = tx_year + data['exchanges']['year'] + data['e-commerce']['year']
        total_total = tx_total + data['exchanges']['total'] + data['e-commerce']['total']

	ex_day = format_float(data['e-commerce']['day'])
        ex_week = format_float(data['e-commerce']['week'])
	ex_month = format_float(data['e-commerce']['month'])
        ex_year = format_float(data['e-commerce']['year'])
	ex_total = format_float(data['e-commerce']['total'])

	to_day = format_float(total_day)
	to_week = format_float(total_week)
	to_month = format_float(total_month)
	to_year = format_float(total_year)
	to_total = format_float(total_total)

        body += ['<tr><td class="td_header">Volume last 24 hours</td><td>', Tx_day, '</td><td>', usd_day, '</td><td>', ex_day, '</td><td>', to_day, '</td></tr>',
		'<tr><td class="td_header">Volume last week</td><td>', Tx_week, '</td><td>', usd_week, '</td><td>', ex_week, '</td><td>', to_week,  '</td></tr>',
		'<tr> <td class="td_header">Volume last month</td><td>', Tx_month, '</td><td>', usd_month, '</td><td>', ex_month, '</td><td>', to_month,  '</td></tr>',
		'<tr> <td class="td_header">Volume last year</td><td>', Tx_year, '</td><td>', usd_year, '</td><td>', ex_year, '</td><td>', to_year,  '</td></tr>',
		'<tr><td class = "td_header">Volume from beginning</td><td> ', Tx_total, '</td><td>', usd_total, '</td><td>', ex_total, '</td><td>', to_total,  ' </td> </tr>\n']
